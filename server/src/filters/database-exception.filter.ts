...
export class DatabaseExceptionFilter implements ExceptionFilter {
		...
		if (exception.message.includes('duplicate key value violates unique constraint')) {
			const constraintMatch = exception.message.match(/unique constraint "([^"]+)"/);
		...
		} else if (exception.message.includes('relation') && exception.message.includes('does not exist')) {
		...
		} else if (exception.message.includes('syntax error') || exception.message.includes('invalid input syntax')) {
			...
			const constraintMatch = exception.message.match(/foreign key constraint "([^"]+)"/);
			...
			const constraintMatch = exception.message.match(/check constraint "([^"]+)"/);
